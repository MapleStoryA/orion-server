...
function warp(selection){
    if (status == 0)
        ...
        var selStr = "#eSelect your destination.#b";
        for (var i = 0; i < maps[selection].length; i++)
            selStr += "\r\n#L" + i + "##m" + maps[selection][i] + "#";
        cm.sendSimple(selStr);
    } else if (status == 2) {
        chosenMap = selection;
        cm.sendYesNo("#eDo you want to go to #m" + maps[chosenSection][selection] + "#?");
    } else if (status == 3) {
...
function jobAdv(selection){
    if (status == 0) {
        ...
        if (cm.getJobId() % 10 == 2) {
            cm.sendOk("#eHey, how's it going? I've been doing well here.");
            cm.dispose();
            ...
            if ((secondJob && cm.getLevel() < 70) || (!secondJob && cm.getLevel() < 120)) {
                ...
                cm.sendYesNo("#eGreat job getting to level " + cm.getLevel() + ". Would you like to become a #b"+cm.getJobName(newJob)+"#k ?");
        } else {
            ...
            if (possibleJobs.length == 0) {
                ...
                var text = "#eThere are the available jobs you can take#b";
