...
function action(mode, type, selection) {
	//cm.debug('mode: ' + mode + ' type:' + type + ' seleciton: ' + selection
	//		+ ' status:' + status);
	//cm.debug('Map: ' + cm.getPlayer().getMapId());
	if (cm.getPlayer().getMapId() == 211042200) {
		...
		case 0:
			if (cm.getPlayer().getLevel() < 100) {
				cm
						.sendOk("There is a level requirement of 100 to attempt Chaos Zakum.");
				cm.dispose();
				return;
			}
			if (cm.getPlayer().getClient().getChannel() != 2) {
				cm.sendOk("Chaos Zakum may only be attempted on channel 2.");
				cm.dispose();
				return;
			}
			...
			if (prop == null || prop.equals("0")) {
				...
				if (squadAvailability == -1) {
					status = 1;
					cm
							.sendYesNo("Are you interested in becoming the leader of the expedition Squad?");
					...
					if (type == -1) {
						...
						if (memberType == 2) {
							...
							status = 5;
							cm
									.sendSimple("What do you want to do? \r\n#b#L0#Check out members#l \r\n#b#L1#Join the squad#l \r\n#b#L2#Withdraw from squad#l");
						} else if (memberType == -1) {
							cm
									.sendOk("The squad has ended, please re-register.");
							cm.safeDispose();
						} else {
							status = 5;
							cm
					...
					if (eim == null) {
						cm
								.sendOk("The squad's battle against the boss has already begun.");
						cm.safeDispose();
					} else {
						cm
								.sendYesNo("Ah, you have returned. Would you like to join your squad in the fight again?");
						status = 2;
					}
				}
			} else {
			...
			if (mode == 1) {
				if (cm
						.registerSquad(
								"ChaosZak",
								5,
								" has been named the Leader of the squad (Chaos). If you would you like to join please register for the Expedition Squad within the time period.")) {
					cm
							.sendOk("You have been named the Leader of the Squad. For the next 5 minutes, you can add the members of the Expedition Squad.");
				} else {
					cm.sendOk("An error has occurred adding your squad.");
				}
			} else {
				cm
						.sendOk("Talk to me if you want to become the leader of the Expedition squad.")
			}
			cm.safeDispose();
			break;
		case 2:
		...
		case 5:
			if (selection == 0) {
				if (!cm.getSquadList("ChaosZak", 0)) {
					cm
							.sendOk("Due to an unknown error, the request for squad has been denied.");
					cm.safeDispose();
				} else {
					cm.dispose();
				}
				...
				if (ba == 2) {
					cm
							.sendOk("The squad is currently full, please try again later.");
					cm.safeDispose();
				} else if (ba == 1) {
				...
				if (baa == 1) {
					cm
							.sendOk("You have withdrawed from the squad successfully");
					cm.safeDispose();
				} else {
		...
		case 0:
			if (cm.getPlayer().getLevel() < 50) {
				cm
						.sendOk("There is a level requirement of 50 to attempt Zakum.");
				cm.dispose();
				return;
			}
			if (cm.getPlayer().getClient().getChannel() != 3
					&& cm.getPlayer().getClient().getChannel() != 2) {
			...
			if (mode == 1) {
				if (cm
						.registerSquad(
								"ZAK",
								5,
								" has been named the Leader of the squad (Regular). If you would you like to join please register for the Expedition Squad within the time period.")) {
					cm
							.sendOk("You have been named the Leader of the Squad. For the next 5 minutes, you can add the members of the Expedition Squad.");
				} else {
					cm.sendOk("An error has occurred adding your squad.");
