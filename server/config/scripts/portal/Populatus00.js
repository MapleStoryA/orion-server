function enter(pi) {
	if (pi.getPlayer().getClient().getChannel() != 1
			&& pi.getPlayer().getClient().getChannel() != 2) {
		pi.playerMessage(5,
				"This boss may only be attempted on channel 1 and 2");
		return false;
	}
	...
	if (pi.getPlayerCount(220080001) <= 0) { // Papu Map
		...
		if (pi.getMap(220080001).getSpeedRunStart() == 0
				&& (pi.getMonsterCount(220080001) <= 0 || pi.getMap(220080001)
						.isDisconnected(pi.getPlayer().getId()))) {
			pi.playPortalSE();
			pi.warp(220080001, "st00");
			return true;
		} else {
			pi
					.playerMessage(
							5,
							"The battle against Papulatus has already begun, so you may not enter this place.");
			return false;
		}
	}
	...
	if (!canEnter) {
		pi.playerMessage(5,
				"You may only enter the Origin of Clocktower twice per day.");
		return false;
	}
}