...
public class MapleShop {
    ...
    public void buy(MapleClient c, int itemId, short quantity, byte slot) {
        ...
        if (item != null && item.getPrice() > 0 && item.getReqItem() == 0) {
            ...
            final int tprice = (ServerConstants.SHOP_DISCOUNT && ii.getItemEffect(item.getItemId()).isPotion() && !ii.getItemEffect(item.getItemId()).isSkill()) ? (price - Math.round((price * ServerConstants.SHOP_DISCOUNT_PERCENT) / 100f)) : price;
