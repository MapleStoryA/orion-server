...
import client.drop.MapleDropData;
import client.drop.MapleDropProvider;
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (target != null) {
                ...
                if (MapleCharacter.ban(
                    ...
                    c.getPlayer()
                            .dropMessage(6, "[" + getCommand() + "] Successfully offline banned " + splitted[1] + ".");
                    return 1;
                } else {
    ...
    public static class StartEvent extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (c.getChannelServer().getEvent() == c.getPlayer().getMapId()) {
                ...
                c.getPlayer()
                        .dropMessage(
                                5, "!scheduleevent must've been done first, and you must be in the" + " event map.");
                return 0;
            }
        }
    }
            ...
            if (npc != null && !npc.getName().equals("MISSINGNO")) {
                ...
                try (var con = DatabaseConnection.getConnection()) {
                    try (PreparedStatement ps =
                            con.prepareStatement("INSERT INTO wz_customlife (dataid, f, hide, fh, cy, rx0, rx1,"
                                    + " type, x, y, mid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?,"
                                    + " ?)")) {
                        ps.setInt(1, npcId);
