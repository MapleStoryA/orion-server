...
public class UseCashItemHandler extends AbstractMaplePacketHandler {
    ...
    public static void PickupItemAtSpot(
            final MapleClient c, final MapleCharacter chr, final MapleMapObject ob) {
        if (ob == null) {
    ...
    public void handlePacket(CInPacket packet, final MapleClient c) {
        ...
        switch (itemId) {
            ...
            case 5043000:
                { // NPC Teleport Rock
                    final short questid = packet.readShort();
                    final int npcid = packet.readInt();
                    final MapleQuest quest = MapleQuest.getInstance(questid);

                    if (c.getPlayer().getQuest(quest).getStatus() == 1
                            && quest.canComplete(c.getPlayer(), npcid)) {
                        ...
                        if (mapId != -1) {
                            ...
                            if (map.containsNPC(npcid)
                                    && !FieldLimitType.VipRock.check(
                                            c.getPlayer().getMap().getFieldLimit())
            ...
            case 5040001:
                { // Teleport Coke
                    if (packet.readByte() == 0) { // Rocktype
                        final MapleMap destination_map =
                                c.getChannelServer().getMapFactory().getMap(packet.readInt());
                        boolean has_registered_map;
                        if (itemId == VIP_TELEPORT_ROCK) {
                            has_registered_map =
                                    c.getPlayer()
                                            .getVipTeleportRock()
                                            .hasMap(destination_map.getId());
                        } else {
                            has_registered_map =
                                    c.getPlayer()
                                            .getRegTeleportRock()
                        ...
                        if (victim != null
                            ...
                            if (!FieldLimitType.VipRock.check(
                                            c.getPlayer().getMap().getFieldLimit())
                                    && !FieldLimitType.VipRock.check(
                                            c.getChannelServer()
                                                    .getMapFactory()
            ...
            case 5050000:
                { // AP Reset
                    List<Pair<MapleStat, Integer>> statupdate =
                    ...
                    switch (apfrom) { // AP to
                        case 64: // str
                            if (playerst.getStr() <= 4
                                    || (c.getPlayer().getJob().getId() % 1000 / 100 == 1
                                            && playerst.getStr() <= 35)) {
                                used = false;
                            }
                            break;
                        case 128: // dex
                            if (playerst.getDex() <= 4
                                    || (c.getPlayer().getJob().getId() % 1000 / 100 == 3
                                            && playerst.getStr() <= 25)
                                    || (c.getPlayer().getJob().getId() % 1000 / 100 == 4
                                            && playerst.getLuk() <= 25)
                                    || (c.getPlayer().getJob().getId() % 1000 / 100 == 5
                                            && playerst.getStr() <= 20)) {
                                used = false;
                            }
                            break;
                        case 256: // int
                            if (playerst.getInt() <= 4
                                    || (c.getPlayer().getJob().getId() % 1000 / 100 == 2
                                            && playerst.getInt() <= 20)) {
                                used = false;
                            }
                            break;
                        ...
                        switch (apto) { // AP to
                            case 64:
                                { // str
                                    ...
                                    statupdate.add(
                                            new Pair<MapleStat, Integer>(MapleStat.STR, toSet));
                                    break;
                                }
                            case 128:
                                { // dex
                                    ...
                                    statupdate.add(
                                            new Pair<MapleStat, Integer>(MapleStat.DEX, toSet));
                                    break;
                                }
                            case 256:
                                { // int
                                    ...
                                    statupdate.add(
                                            new Pair<MapleStat, Integer>(MapleStat.INT, toSet));
                                    break;
                                }
                            case 512:
                                { // luk
                                    ...
                                    statupdate.add(
                                            new Pair<MapleStat, Integer>(MapleStat.LUK, toSet));
                                    break;
                                }
                            case 2048: // hp
                                ...
                                if (job == 0) { // Beginner
                                    ...
                                    int improvingMaxHPLevel =
                                            c.getPlayer().getSkillLevel(improvingMaxHP);
                                ...
                                c.getPlayer()
                                        .setHpApUsed((short) (c.getPlayer().getHpApUsed() + 1));
                                playerst.setMaxHp(maxhp);
                                statupdate.add(
                                        new Pair<MapleStat, Integer>(MapleStat.MAXHP, maxhp));
                                break;
                            ...
                            case 8192: // mp
                                ...
                                if (job == 0) { // Beginner
                                    ...
                                    int improvingMaxMPLevel =
                                            c.getPlayer().getSkillLevel(improvingMaxMP);
                                ...
                                statupdate.add(
                                        new Pair<MapleStat, Integer>(MapleStat.MAXMP, maxmp));
                                break;
                        }
                        ...
                        c.getSession()
                                .write(
                                        MaplePacketCreator.updatePlayerStats(
                                                statupdate, true, c.getPlayer().getJob().getId()));
                    }
                    break;
                }
            ...
            case 5050009:
                {
                    ...
                    if (GameConstants.getSkillBookForSkill(skill1)
                            != GameConstants.getSkillBookForSkill(skill2)) { // resistance
                        // evan
                        break;
                    }
                    if (GameConstants.getJobNumber(skill1 / 10000)
                            > GameConstants.getJobNumber(skill2 / 10000)) { // putting
                        // 3rd
                        // job
                        // skillpoints
                        // into
            ...
            case 5062000:
                { // miracle cube
                    ...
                    if (item != null
                            && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot()
                                    >= 1) {
                        ...
                        if (eq.getState() >= 5) {
                            ...
                            c.getSession()
                                    .write(
                                            MaplePacketCreator.scrolledItem(
                                                    toUse, item, false, true));
                            c.getPlayer()
                                    .getMap()
                                    .broadcastMessage(
                                            MaplePacketCreator.getMiracleCubeEffect(
            ...
            case 5520000:
                { // Karma
                    final MapleInventoryType type =
                            MapleInventoryType.getByType((byte) packet.readInt());
                    final IItem item =
                            c.getPlayer().getInventory(type).getItem((byte) packet.readInt());

                    if (item != null
                            && !ItemFlag.KARMA_EQ.check(item.getFlag())
                            && !ItemFlag.KARMA_USE.check(item.getFlag())) {
                        if ((itemId == 5520000
                                        && MapleItemInformationProvider.getInstance()
                                                .isKarmaEnabled(item.getItemId()))
                                || (itemId == 5520001
                                        && MapleItemInformationProvider.getInstance()
                                                .isPKarmaEnabled(item.getItemId()))) {
            ...
            case 5570000:
                { // Vicious Hammer
                    packet.readInt(); // Inventory type, Hammered eq is always EQ.
                    final Equip item =
                            (Equip)
                                    c.getPlayer()
                                            .getInventory(MapleInventoryType.EQUIP)
                    ...
                    if (item != null) {
                        if (GameConstants.canHammer(item.getItemId())
                            ...
                            c.getSession()
                                    .write(
                                            MTSCSPacket.ViciousHammer(
                                                    true, item.getViciousHammer()));
                            used = true;
                        } else {
            ...
            case 5610000:
                { // Vega 30
                    packet.readInt(); // Inventory type, always eq
                    final byte dst = (byte) packet.readInt();
                    packet.readInt(); // Inventory type, always use
                    final byte src = (byte) packet.readInt();
                    used =
                            InventoryHandlerUtils.UseUpgradeScroll(
                                    src, dst, (byte) 0, c, c.getPlayer(), itemId, (byte) 0);
            ...
            case 5061000:
                { // Sealing Lock 7 days
                    ...
                    if (item != null && item.getExpiration() == -1) {
                        ...
                        flag |= ItemFlag.LOCK.getValue();
                        item.setFlag(flag);
                        long expiration = new Date().getTime() + (7 * 1000L * 60 * 60 * 24);
            ...
            case 5061001:
                { // Sealing Lock 30 days
                    ...
                    if (item != null && item.getExpiration() == -1) {
                        ...
                        flag |= ItemFlag.LOCK.getValue();
                        item.setFlag(flag);
                        long expiration = new Date().getTime() + (30 * 1000L * 60 * 60 * 24);
                        item.setExpiration(expiration);
            ...
            case 5061003:
                { // Sealing Lock year
                    ...
                    if (item != null && item.getExpiration() == -1) {
                        ...
                        long expiration = new Date().getTime() + (365 * 1000L * 60 * 60 * 24);
                        item.setExpiration(expiration);
            ...
            case 5060003:
                { // peanut
                    IItem item =
                            c.getPlayer().getInventory(MapleInventoryType.ETC).findById(4170023);
                    if (item == null || item.getQuantity() <= 0) { // hacking{
                        return;
                    }
                    if (InventoryHandlerUtils.getIncubatedItems(c)) {
                        MapleInventoryManipulator.removeFromSlot(
                                c, MapleInventoryType.ETC, item.getPosition(), (short) 1, false);
                        used = true;
                    }
                }
                break;
            case 5070000:
                { // Megaphone
                    ...
                    if (!c.getChannelServer().getMegaphoneMuteState()) {
                        final String message = packet.readMapleAsciiString();
                        ...
                        c.getPlayer()
                                .getMap()
                                .broadcastMessage(
                                        MaplePacketCreator.serverNotice(2, sb.toString()));
                        used = true;
                    } else {
                        c.getPlayer()
                                .dropMessage(5, "The usage of Megaphone is currently disabled.");
                    }
                    break;
                }
            case 5071000:
                { // Megaphone
                    ...
                    if (!c.getChannelServer().getMegaphoneMuteState()) {
                        ...
                        c.getChannelServer()
                                .broadcastPacket(MaplePacketCreator.serverNotice(2, sb.toString()));
                        used = true;
                    } else {
                        c.getPlayer()
                                .dropMessage(5, "The usage of Megaphone is currently disabled.");
            ...
            case 5077000:
                { // 3 line Megaphone
                    ...
                    if (!c.getChannelServer().getMegaphoneMuteState()) {
                        final byte numLines = packet.readByte();
                        if (numLines > 3) {
                            return;
                        }
                        ...
                        final boolean ear = packet.readByte() > 0;

                        BroadcastHelper.broadcastSmega(
                                MaplePacketCreator.tripleSmega(messages, ear, c.getChannel()));
                        used = true;
                    } else {
                        c.getPlayer()
                                .dropMessage(5, "The usage of Megaphone is currently disabled.");
                    ...
                    if (!c.getChannelServer().getMegaphoneMuteState()) {
                        ...
                        BroadcastHelper.broadcastSmega(
                                MaplePacketCreator.serverNotice(
                                        20, c.getChannel(), sb.toString(), ear));
                        used = true;
                    } else {
                        c.getPlayer()
                                .dropMessage(5, "The usage of Megaphone is currently disabled.");
            ...
            case 5072000:
                { // Super Megaphone
                    ...
                    if (!c.getChannelServer().getMegaphoneMuteState()) {
                        ...
                        BroadcastHelper.broadcastSmega(
                                MaplePacketCreator.serverNotice(
                                        3, c.getChannel(), sb.toString(), ear));
                        used = true;
                    } else {
                        c.getPlayer()
                                .dropMessage(5, "The usage of Megaphone is currently disabled.");
                ...
                { // Item Megaphone
                    ...
                    if (!c.getChannelServer().getMegaphoneMuteState()) {
                        ...
                        if (packet.readByte() == 1) { // item
                            byte invType = (byte) packet.readInt();
                            ...
                            item =
                                    c.getPlayer()
                                            .getInventory(MapleInventoryType.getByType(invType))
                                            .getItem((byte) pos);
                        }
                        BroadcastHelper.broadcastSmega(
                                MaplePacketCreator.itemMegaphone(
                                        sb.toString(), ear, c.getChannel(), item));
                        used = true;
                    } else {
                        c.getPlayer()
                                .dropMessage(5, "The usage of Megaphone is currently disabled.");
            ...
            case 5075002:
                { // MapleTV Heart Messenger
                    c.getPlayer()
                            .dropMessage(5, "There are no MapleTVs to broadcast the message to.");
                    break;
                }
            case 5075003:
            case 5075004:
            case 5075005:
                {
                    ...
                    if (tvType == 3) {
                        packet.readByte(); // who knows
                    }
                    boolean ear = tvType != 1 && tvType != 2 && packet.readByte() > 1; // for
                    // tvType
                    // 1/2,
                    // there
                    // is
            ...
            case 5090000:
                { // Note
                    final String sendTo = packet.readMapleAsciiString();
            ...
            case 5100000:
                { // Congratulatory Song
                    c.getPlayer()
                            .getMap()
                            .broadcastMessage(
                                    MaplePacketCreator.musicChange("Jukebox/Congratulation"));
                    used = true;
                    break;
                }
            case 5170000:
                { // Pet name change
                    ...
                    if (MapleCharacterHelper.canChangePetName(nName)) {
                        ...
                        c.getPlayer()
                                .getMap()
                                .broadcastMessage(
                                        MTSCSPacket.changePetName(c.getPlayer(), nName, slo));
                        used = true;
                    }
                    break;
                }
            ...
            case 5240028:
                { // Pet food
                    ...
                    if (pet.getCloseness() < 30000) {
                        ...
                        if (pet.getCloseness()
                                >= GameConstants.getClosenessNeededForLevel(pet.getLevel() + 1)) {
                            pet.setLevel(pet.getLevel() + 1);
                            c.getSession()
                                    .write(
                                            PetPacket.showOwnPetLevelUp(
                                                    c.getPlayer().getPetIndex(pet)));
                            c.getPlayer()
                                    .getMap()
                                    .broadcastMessage(
                                            PetPacket.showPetLevelUp(c.getPlayer(), petindex));
                        }
                    }
                    c.getSession()
                            .write(
                    ...
                    c.getPlayer()
                            ...
                            .broadcastMessage(
                                    ...
                                    PetPacket.commandResponse(
                                            c.getPlayer().getId(), (byte) 1, petindex, true, true),
                                    true);
                    used = true;
                    break;
                }
                ...
                { // Passed gas
                    Rectangle bounds =
                            new Rectangle(
                                    ...
                                    (int) c.getPlayer().getPosition().getY(),
                                    1,
                                    1);
            ...
            case 5370000:
                { // Chalkboard
                    c.getPlayer().setChalkboard(packet.readMapleAsciiString());
                    break;
                }
            case 5370001:
                { // BlackBoard
            ...
            case 5390008:
                { // soccer Messenger
                    ...
                    if (!c.getChannelServer().getMegaphoneMuteState()) {
                        ...
                        final boolean ear = packet.readByte() != 0;
                        BroadcastHelper.broadcastSmega(
                                MaplePacketCreator.getAvatarMega(
                                        c.getPlayer(), c.getChannel(), itemId, lines, ear));
                        used = true;
                    } else {
                        c.getPlayer()
                                .dropMessage(5, "The usage of Megaphone is currently disabled.");
            ...
            default:
                if (itemId / 10000 == 512) {
                    ...
                    if (buff != 0) {
                        for (MapleCharacter mChar :
                                c.getPlayer().getMap().getCharactersThreadsafe()) {
                    ...
                    if (isMapleCoin) {
                        ...
                        c.getPlayer()
                                .dropMessage(
                                        1, "You've gained " + totalMaplePoints + " maple points");
                        BroadcastHelper.broadcastMessage(
                                MaplePacketCreator.serverNotice(
                                        6,
                                        itemId,
                                        "A lucky person just used a {"
                                                + MapleItemInformationProvider.getInstance()
                                                        .getName(itemId)
                                                + "}"));
                        c.enableActions();
                    ...
                    InventoryHandlerUtils.UseSkillBook(
                            slot, itemId, c, c.getPlayer()); // this should
                    // handle
                    // removing
        ...
        if (used) {
            MapleInventoryManipulator.removeFromSlot(
                    c, MapleInventoryType.CASH, slot, (short) 1, true, false);
        }
        c.getSession().write(MaplePacketCreator.enableActions());
        if (cc) {
            ...
            c.getPlayer()
                    .changeChannel(
                            c.getChannel() == WorldServer.getInstance().getChannelCount()
                                    ? 1
                                    : (c.getChannel() + 1));
        }
    }
