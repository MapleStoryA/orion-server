    ...
    public void handlePacket(InPacket packet, final MapleClient c) {
        ...
        switch (itemId) {
            ...
            case 5040001: { // Teleport Coke
                if (packet.readByte() == 0) { // Rocktype
                    ...
                    final MapleCharacter victim =
                            c.getChannelServer().getPlayerStorage().getCharacterByName(packet.readMapleAsciiString());
                    if (victim != null
                            && !victim.isGameMaster()
                            && c.getPlayer().getEventInstance() == null
                            && victim.getEventInstance() == null) {
                        if (!FieldLimitType.VipRock.check(c.getPlayer().getMap().getFieldLimit())
                                && !FieldLimitType.VipRock.check(c.getChannelServer()
                                        .getMapFactory()
                                        .getMap(victim.getMapId())
                                        .getFieldLimit())) {
                            if (itemId == 5041000
                                    || (victim.getMapId() / 100000000)
                                            == (c.getPlayer().getMapId() / 100000000)) { // Viprock
                                // or
                                // same
                                // continent
                                c.getPlayer()
                ...
                switch (apfrom) { // AP to
                    case 64: // str
                        if (playerst.getStr() <= 4
                                || (c.getPlayer().getJob().getId() % 1000 / 100 == 1 && playerst.getStr() <= 35)) {
                            used = false;
                        }
                        break;
                    case 128: // dex
                        if (playerst.getDex() <= 4
                                || (c.getPlayer().getJob().getId() % 1000 / 100 == 3 && playerst.getStr() <= 25)
                    ...
                    case 256: // int
                        if (playerst.getInt() <= 4
                                || (c.getPlayer().getJob().getId() % 1000 / 100 == 2 && playerst.getInt() <= 20)) {
                            used = false;
                        }
                        break;
                    case 512: // luk
            ...
            case 5060000: { // Item Tag
                final IItem item =
                        c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).getItem(packet.readByte());
            ...
            case 5062000: { // miracle cube
                final IItem item =
                        c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem((byte) packet.readInt());
                if (item != null
            ...
            case 5520000: { // Karma
                final MapleInventoryType type = MapleInventoryType.getByType((byte) packet.readInt());
                final IItem item = c.getPlayer().getInventory(type).getItem((byte) packet.readInt());
            ...
            case 5077000: { // 3 line Megaphone
                ...
                if (!c.getChannelServer().getMegaphoneMuteState()) {
                    ...
                    Broadcast.broadcastSmega(MaplePacketCreator.tripleSmega(messages, ear, c.getChannel()));
                    used = true;
                } else {
                    c.getPlayer().dropMessage(5, "The usage of Megaphone is currently disabled.");
                }
            ...
            case 5074000: { // Skull Megaphone
                ...
                if (!c.getChannelServer().getMegaphoneMuteState()) {
                    ...
                    Broadcast.broadcastSmega(MaplePacketCreator.serverNotice(20, c.getChannel(), sb.toString(), ear));
                    used = true;
                } else {
                    c.getPlayer().dropMessage(5, "The usage of Megaphone is currently disabled.");
                }
            ...
            case 5075005: {
                ...
                Broadcast.broadcastSmega(MaplePacketCreator.serverNotice(
                        3, c.getChannel(), c.getPlayer().getName() + " : " + message, ear));
                used = true;
                break;
            }
            ...
            case 5240028: { // Pet food
                ...
                c.getPlayer()
                        ...
                        .broadcastMessage(
                                c.getPlayer(),
                                PetPacket.commandResponse(c.getPlayer().getId(), (byte) 1, petindex, true, true),
                                true);
                used = true;
                break;
            }
            ...
            default:
                if (itemId / 10000 == 512) {
                    ...
                    final int mesars =
                            MapleItemInformationProvider.getInstance().getMeso(itemId);
        ...
        if (cc) {
            ...
            c.getPlayer()
                    .changeChannel(
                            c.getChannel() == WorldServer.getInstance().getChannelCount() ? 1 : (c.getChannel() + 1));
        }
    }
}
