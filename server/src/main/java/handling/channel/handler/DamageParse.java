                        ...
                        case 21120010: { // (hidden) Overswing - Triple Attack
                            if (player.getBuffedValue(MapleBuffStat.WK_CHARGE) != null && !monster.getStats().isBoss()) {
                                ...
                                if (eff != null && eff.getSourceId() == 21111005) {
                                    monster.applyStatus(player, new MonsterStatusEffect(MonsterStatus.SPEED, eff.getX(), eff.getSourceId(), null, false), false, eff.getY() * 1000L, false);
                                }
                            }
                            if (player.getBuffedValue(MapleBuffStat.BODY_PRESSURE) != null && !monster.getStats().isBoss()) {
                                ...
                                if (eff != null && eff.makeChanceResult() && !monster.isBuffed(MonsterStatus.NEUTRALISE)) {
                                    monster.applyStatus(player, new MonsterStatusEffect(MonsterStatus.NEUTRALISE, 1, eff.getSourceId(), null, false), false, eff.getX() * 1000L, false);
                                }
                            }
                            break;
                        }
                        ...
                        if (player.getJob().getId() == 121) { // WHITEKNIGHT
                            for (int charge : charges) {
                                ...
                                if (player.isBuffFrom(MapleBuffStat.WK_CHARGE, skill)) {
                                    ...
                                    monster.applyStatus(player, monsterStatusEffect, false, skill.getEffect(player.getSkillLevel(skill)).getY() * 2000L, false);
                                    break;
                                }
                            }
                        }
        ...
        for (final Pair<Integer, Boolean> eachde : oned.getAttack()) {
            ...
            if (s.makeChanceResult()) {
                ...
                MapTimer.getInstance().schedule(new Runnable() {
                    ...
                    @Override
                    public void run() {
                        player.getMap().spawnMesoDrop(Math.min((int) Math.max(((double) eachd / (double) 20000) * (double) maxmeso, 1), maxmeso), new Point((int) (mob.getPosition().getX() + Randomizer.nextInt(100) - 50), (int) (mob.getPosition().getY())), mob, player, true, (byte) 0);
                    }
                }, 100);
            }
        }
        ...
        if (JobConstants.isEvanSkill(attack.skill) || attack.skill != 4211006 && attack.skill != 3211003 && attack.skill != 4111004 && (CriticalRate > 0 || attack.skill == 4221001 || attack.skill == 3221007)) { //blizz + shadow meso + m.e no crits
            for (AttackPair p : attack.allDamage) {
                if (p.getAttack() != null) {
                    ...
                    final List<Pair<Integer, Boolean>> eachd_copy = new ArrayList<Pair<Integer, Boolean>>(p.getAttack());
                    for (Pair<Integer, Boolean> eachd : p.getAttack()) {
                        hit++;
                        if (!eachd.right) {
