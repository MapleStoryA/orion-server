        ...
        switch (action) { // Mode
            ...
            case CHAT: {
                ...
                if (chr.getTrade() != null) {
                    ...
                    ips.broadcastToVisitors(
                            PlayerShopPacket.shopChat(chr.getName() + " : " + text, ips.getVisitorSlot(chr)));
                }
                break;
            }
            case EXIT: {
                ...
                if (ivItem != null) {
                    ...
                    if (ivItem.getQuantity() >= bundles_perbundle) {
                        ...
                        if (bundles_perbundle >= 50 && GameConstants.isUpgradeScroll(ivItem.getItemId())) {
                            ...
                            final String msg = "[PLAYER_SHOP_ADD_ITEM / ADD_ITEM] Placed "
                                    + bundles_perbundle
                                    + " of "
                                    + ivItem.getItemId();
                            log.info(file + " : " + msg);
            ...
            case BUY_ITEM_HIREDMERCHANT: { // Buy and Merchant buy
                ...
                if (quantity >= 50
                    ...
                    final String file = chr.getName();
                    final String msg = "[BUY_ITEM_PLAYER_SHOP / BUY_ITEM_STORE | BUY_ITEM_HIREDMERCHANT]"
                            + " Placed "
                            + quantity
                            + " of "
                ...
                if (item != null) {
                    if (item.getBundles() > 0) {
                        ...
                        if (item_get.getQuantity() >= 50
                            ...
                            final String file = chr.getName();
                            final String msg = "[REMOVE_ITEM] Removed "
                        ...
                        if (MapleInventoryManipulator.checkSpace(
                                c, item_get.getItemId(), item_get.getQuantity(), item_get.getOwner())) {
            ...
            case MAINTANCE_ORGANISE: {
                ...
                if (imps != null && imps.isOwner(chr) && !(imps instanceof MapleMiniGame)) {
                    for (Iterator<MaplePlayerShopItem> iterator =
                                    imps.getItems().iterator();
                            iterator.hasNext(); ) {
                ...
                if (ips != null && ips instanceof MapleMiniGame game) {
                    if (!game.isOwner(chr) && game.isOpen()) {
                        ...
                        game.broadcastToVisitors(
                                PlayerShopPacket.getMiniGameReady(game.isReady(game.getVisitorSlot(chr))));
                    }
                }
                break;
            }
                ...
                if (ips != null && ips instanceof MapleMiniGame game) {
                    ...
                    if (game.getRequestedTie() > -1 && game.getRequestedTie() != game.getVisitorSlot(chr)) {
                        if (packet.readByte() > 0) {
                            game.broadcastToVisitors(
                                    PlayerShopPacket.getMiniGameResult(game, 1, game.getRequestedTie()));
                            game.nextLoser();
                            game.setOpen(true);
                            game.update();
                            game.checkExitAfterGame();
            ...
            case SKIP: {
                ...
                if (ips != null && ips instanceof MapleMiniGame game) {
                    ...
                    if (game.getLoser() != ips.getVisitorSlot(chr)) {
                        ips.broadcastToVisitors(PlayerShopPacket.shopChat(
