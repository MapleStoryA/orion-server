...
public class PlayerInteractionHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(CInPacket packet, MapleClient c) {
        ...
        switch (action) { // Mode
            case CREATE:
                {
                    ...
                    final byte createType = packet.readByte();
                    if (createType == 3) { // trade
                        ...
                        final String desc = packet.readMapleAsciiString();
                        String pass = "";
                        if (packet.readByte() > 0 && (createType == 1 || createType == 2)) {
                            pass = packet.readMapleAsciiString();
                        }
                        if (createType == 1 || createType == 2) {
                            final int piece = packet.readByte();
                            ...
                            MapleMiniGame game =
                                    new MapleMiniGame(
                                            chr, itemId, desc, pass, createType); // itemid
                            ...
                            IItem shop =
                                    c.getPlayer()
                                            ...
                                            .getItem((byte) packet.readShort());
                            if (shop == null
                                    || shop.getQuantity() <= 0
                                    || shop.getItemId() != packet.readInt()
                                    || c.getPlayer().getMapId() < 910000001
                                    || c.getPlayer().getMapId() > 910000022) {
                                return;
                            }
                            if (createType == 4) {
                                MaplePlayerShop mps =
                                        new MaplePlayerShop(chr, shop.getItemId(), desc);
                                chr.setPlayerShop(mps);
                                chr.getMap().addMapObject(mps);
                                c.getSession().write(PlayerShopPacket.getPlayerStore(chr, true));
                            } else {
                                final HiredMerchant merch =
                                        new HiredMerchant(chr, shop.getItemId(), desc);
                                chr.setPlayerShop(merch);
                                chr.getMap().addMapObject(merch);
                                c.getSession()
                                        .write(PlayerShopPacket.getHiredMerch(chr, merch, true));
                            }
                        }
                    }
                    break;
            ...
            case INVITE_TRADE:
                {
                    ...
                    MapleTrade.inviteTrade(chr, chr.getMap().getCharacterById(packet.readInt()));
                    break;
                }
            case DENY_TRADE:
                {
            ...
            case VISIT:
                {
                    ...
                    if (chr.getTrade() != null && chr.getTrade().getPartner() != null) {
                        ...
                        final int obid = packet.readInt();
                        MapleMapObject ob =
                                chr.getMap().getMapObject(obid, MapleMapObjectType.HIRED_MERCHANT);
                        if (ob == null) {
                            ...
                            if (ob instanceof HiredMerchant) {
                                ...
                                if (merchant.isOwner(chr)) {
                                    ...
                                    if (!merchant.isOpen() || !merchant.isAvailable()) {
                                        chr.dropMessage(
                                                1,
                                                "This shop is in maintenance, please come by"
                                                        + " later.");
                                        ...
                                        if (ips.getFreeSlot() == -1) {
                                            ...
                                            chr.dropMessage(
                                                    1, "You have been banned from this store.");
                                        } else {
                                            chr.setPlayerShop(ips);
                                ...
                                if (ips instanceof MaplePlayerShop
                                        && ((MaplePlayerShop) ips).isBanned(chr.getName())) {
                                    chr.dropMessage(1, "You have been banned from this store.");
                                    return;
                                } else {
                                    if (ips.getFreeSlot() < 0
                                            || ips.getVisitorSlot(chr) > -1
                                            || !ips.isOpen()
                                            || !ips.isAvailable()) {
                                        c.getSession().write(PlayerShopPacket.getMiniGameFull());
                                    } else {
                                        if (packet.available() > 0 && packet.readByte() > 0) { // a
                                            // password
                                            // has
                                            // been
                                            // entered
                                            String pass = packet.readMapleAsciiString();
                                        ...
                                        if (ips instanceof MapleMiniGame) {
                                            ...
                                            c.getSession()
                                                    .write(
                                                            PlayerShopPacket.getPlayerStore(
                                                                    chr, false));
                                        }
                                    }
                                }
            ...
            case CHAT:
                {
                    ...
                    if (chr.getTrade() != null) {
                        chr.getTrade().chat(packet.readMapleAsciiString());
                        ...
                        if (ips.isOwner(chr)) {
                            c.getSession()
                                    .write(
                                            PlayerShopPacket.shopChat(
                                                    chr.getName() + " : " + text, 1));
                            return;
                        }
                        ips.broadcastToVisitors(
                                PlayerShopPacket.shopChat(
                                        chr.getName() + " : " + text, ips.getVisitorSlot(chr)));
                    }
                    break;
                }
            case EXIT:
                    ...
                    if (shop != null && shop.isOwner(chr) && shop.getShopType() < 3) {
                        if (chr.getMap().allowPersonalShop()) {
                            ...
                            if (shop.getShopType() == 1) {
                                ...
                                chr.getMap()
                                        .broadcastMessage(
                                                PlayerShopPacket.spawnHiredMerchant(merchant));
                                chr.setPlayerShop(null);
            ...
            case ADD_ITEM:
                {
                    if (packet.available() < 11) {
                        break;
                    }
                    final MapleInventoryType type = MapleInventoryType.getByType(packet.readByte());
                    final byte slot = (byte) packet.readShort();
                    final short bundles = packet.readShort(); // How many in a bundle
                    final short perBundle = packet.readShort(); // Price per bundle
                    ...
                    if (ivItem != null) {
                        ...
                        if (ivItem.getQuantity() >= bundles_perbundle) {
                            ...
                            if (ii.isDropRestricted(ivItem.getItemId())
                            ...
                            if (bundles_perbundle >= 50
                                    && GameConstants.isUpgradeScroll(ivItem.getItemId())) {
                                final String file = chr.getName();
                                final String msg =
                                        "[PLAYER_SHOP_ADD_ITEM / ADD_ITEM] Placed "
                                                + bundles_perbundle
                                                + " of "
                                                + ivItem.getItemId();
                                log.info(file + " : " + msg);
                            ...
                            if (GameConstants.isThrowingStar(ivItem.getItemId())
                                ...
                                MapleInventoryManipulator.removeFromSlot(
                                        c, type, slot, ivItem.getQuantity(), true);

                                shop.addItem(new MaplePlayerShopItem(sellItem, (short) 1, price));
                            } else {
                                MapleInventoryManipulator.removeFromSlot(
                                        c, type, slot, bundles_perbundle, true);
            ...
            case BUY_ITEM_HIREDMERCHANT:
                { // Buy and Merchant buy
                    final int item = packet.readByte();
                    final short quantity = packet.readShort();
                    ...
                    if (shop == null
                            || shop.isOwner(chr)
                            || shop instanceof MapleMiniGame
                            || (shop.getItems().size() - 1) < item) {
                        c.enableActions();
                        return;
                    }
                    ...
                    if (check > 32767
                            || check <= 0
                            || check2 > Integer.MAX_VALUE
                            || check2 <= 0
                            || check3 > Integer.MAX_VALUE
                    ...
                    if (mesosToHold > Integer.MAX_VALUE) {
                        c.getPlayer()
                                .dropMessage(1, "The player possess more mesos than it can hold.");
                        c.enableActions();
                        return;
                    }
                    if (quantity <= 0
                            || tobuy.getBundles() < quantity
                            || (tobuy.getBundles() % quantity != 0
                                    && GameConstants.isEquip(tobuy.getItem().getItemId())) // Buying
                    ...
                    if (quantity >= 50
                            && GameConstants.isUpgradeScroll(tobuy.getItem().getItemId())) {
                        final String file = chr.getName();
                        final String msg =
                                "[BUY_ITEM_PLAYER_SHOP / BUY_ITEM_STORE | BUY_ITEM_HIREDMERCHANT]"
                                        + " Placed "
                                        + quantity
                                        + " of "
            ...
            case REMOVE_ITEM:
                {
                    int slot = packet.readShort(); // 0
                    final IMaplePlayerShop shop = chr.getPlayerShop();

                    if (shop == null
                            || !shop.isOwner(chr)
                            || shop instanceof MapleMiniGame
                            || shop.getItems().size() <= 0
                    ...
                    if (item != null) {
                        if (item.getBundles() > 0) {
                            ...
                            if (item_get.getQuantity() >= 50
                                    && GameConstants.isUpgradeScroll(item.getItem().getItemId())) {
                                final String file = chr.getName();
                                final String msg =
                                        "[REMOVE_ITEM] Removed "
                                                + item_get.getQuantity()
                                                + " of "
            ...
            case MAINTANCE_ORGANISE:
                {
                    ...
                    if (imps != null && imps.isOwner(chr) && !(imps instanceof MapleMiniGame)) {
                        for (Iterator<MaplePlayerShopItem> iterator = imps.getItems().iterator();
                                iterator.hasNext(); ) {
                        ...
                        long totalMesos = chr.getMeso() + imps.getMesos();
                        if (totalMesos < 0 || totalMesos > Integer.MAX_VALUE) {
                            chr.dropMessage(
                                    1,
                                    "You will have more mesos than you can hold. Please put some"
                                            + " money in the bank.");
                            c.getSession().write(MaplePacketCreator.enableActions());
                        } else {
            ...
            case MERCHANT_BLACKLIST_ADD:
                {
                    final IMaplePlayerShop merchant = chr.getPlayerShop();
                    if (merchant != null && merchant.getShopType() == 1 && merchant.isOwner(chr)) {
                        ((HiredMerchant) merchant).addBlackList(packet.readMapleAsciiString());
                    }
                    break;
                }
            case MERCHANT_BLACKLIST_REMOVE:
                {
                    final IMaplePlayerShop merchant = chr.getPlayerShop();
                    if (merchant != null && merchant.getShopType() == 1 && merchant.isOwner(chr)) {
                        ((HiredMerchant) merchant).removeBlackList(packet.readMapleAsciiString());
                    }
                    break;
                }
            case GIVE_UP:
                {
                    ...
                    if (ips != null && ips instanceof MapleMiniGame) {
                        ...
                        game.broadcastToVisitors(
                                PlayerShopPacket.getMiniGameResult(
                                        game, 0, game.getVisitorSlot(chr)));
                        game.nextLoser();
                        game.setOpen(true);
                        game.update();
            ...
            case UN_READY:
                {
                    ...
                    if (ips != null && ips instanceof MapleMiniGame) {
                        ...
                        if (!game.isOwner(chr) && game.isOpen()) {
                            ...
                            game.broadcastToVisitors(
                                    PlayerShopPacket.getMiniGameReady(
                                            game.isReady(game.getVisitorSlot(chr))));
                        }
                    }
                    break;
                }
                    ...
                    if (ips != null && ips instanceof MapleMiniGame) {
                        ...
                        if (game.isOwner(chr) && game.isOpen()) {
                            ...
                            if (game.getGameType() == 1) {
                                game.broadcastToVisitors(
                                        PlayerShopPacket.getMiniGameStart(game.getLoser()));
                            } else {
                                game.broadcastToVisitors(
                                        PlayerShopPacket.getMatchCardStart(game, game.getLoser()));
                            }
                            game.setOpen(false);
                            game.update();
                        }
            ...
            case REQUEST_TIE:
                {
                    ...
                    if (ips != null && ips instanceof MapleMiniGame) {
                        ...
                        if (game.isOwner(chr)) {
                            game.broadcastToVisitors(
                                    PlayerShopPacket.getMiniGameRequestTie(), false);
                        } else {
                            game.getMCOwner()
                                    .getClient()
                                    .getSession()
                                    .write(PlayerShopPacket.getMiniGameRequestTie());
                        }
            ...
            case ANSWER_TIE:
                {
                    ...
                    if (ips != null && ips instanceof MapleMiniGame) {
                        ...
                        if (game.getRequestedTie() > -1
                                && game.getRequestedTie() != game.getVisitorSlot(chr)) {
                            if (packet.readByte() > 0) {
                                game.broadcastToVisitors(
                                        PlayerShopPacket.getMiniGameResult(
                                                game, 1, game.getRequestedTie()));
                                game.nextLoser();
                                game.setOpen(true);
                                game.update();
                                game.checkExitAfterGame();
            ...
            case SKIP:
                {
                    ...
                    if (ips != null && ips instanceof MapleMiniGame) {
                        ...
                        ips.broadcastToVisitors(
                                PlayerShopPacket.getMiniGameSkip(ips.getVisitorSlot(chr)));
                        game.nextLoser();
                    }
                    break;
                }
            case MOVE_OMOK:
                {
                    ...
                    if (ips != null && ips instanceof MapleMiniGame) {
                        ...
                        game.setPiece(packet.readInt(), packet.readInt(), packet.readByte(), chr);
                    }
                    break;
                }
            case SELECT_CARD:
                {
                    ...
                    if (ips != null && ips instanceof MapleMiniGame) {
                        ...
                        if (packet.readByte() != game.getTurn()) {
                            game.broadcastToVisitors(
                                    PlayerShopPacket.shopChat(
                                            "Omok could not be placed by "
                                                    + chr.getName()
                        ...
                        if (turn == 1) {
                            ...
                            if (game.isOwner(chr)) {
                                game.broadcastToVisitors(
                                        PlayerShopPacket.getMatchCardSelect(turn, slot, fs, turn),
                                        false);
                            } else {
                                game.getMCOwner()
                                        .getClient()
                                        .getSession()
                                        .write(
                                                PlayerShopPacket.getMatchCardSelect(
                                                        turn, slot, fs, turn));
                            }
                            ...
                            game.broadcastToVisitors(
                                    PlayerShopPacket.getMatchCardSelect(
                                            turn, slot, fs, game.isOwner(chr) ? 2 : 3));
                            ...
                            game.broadcastToVisitors(
                                    PlayerShopPacket.getMatchCardSelect(
                                            turn, slot, fs, game.isOwner(chr) ? 0 : 1));
            ...
            case CANCEL_EXIT:
                {
                    ...
                    if (ips != null && ips instanceof MapleMiniGame) {
                        ...
                        game.broadcastToVisitors(
                                PlayerShopPacket.getMiniGameExitAfter(game.isExitAfter(chr)));
                    }
                    break;
                }
            default:
