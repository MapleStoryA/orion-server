...
public class InventoryHandlerUtils {
    ...
    public static final boolean UseUpgradeScroll(
        ...
        if (GameConstants.isTablet(scroll.getItemId())) {
            ...
            if (!(ii.canScroll(scroll.getItemId(), toScroll.getItemId()))
                    && !(ii.isArmorScroll(scroll.getItemId(), toScroll.getItemId()))) {
                c.enableActions();
                return false;
            }
        }
        ...
        if (scrolled == null) {
            scrollSuccess = IEquip.ScrollResult.CURSE;
                ...
                || scrolled.getEnhance() > oldEnhance
        ...
        if (whiteScroll) {
            MapleInventoryManipulator.removeFromSlot(
                    c, MapleInventoryType.USE, wscroll.getPosition(), (short) 1, false, false);
        }
        if (vegas > 0) {
            MapleInventoryManipulator.removeFromSlot(
                    c, MapleInventoryType.USE, scroll.getPosition(), (short) 1, false, false);
        }
        ...
        if (type == 0) {
            chr.getMap()
                    .broadcastMessage(
                            chr,
                            MaplePacketCreator.getNormalScrollEffect(
                                    chr.getId(), scrollSuccess, legendarySpirit, whiteScroll),
                            vegas == 0);
        } else if (type == 1) {
            chr.getMap()
                    .broadcastMessage(
                            chr,
                            MaplePacketCreator.getPotentialScrollEffect(
                                    true, chr.getId(), scrollSuccess, legendarySpirit),
                            vegas == 0);
        } else if (type == 2) {
            chr.getMap()
                    .broadcastMessage(
                            chr,
                            MaplePacketCreator.getPotentialScrollEffect(
                                    false, chr.getId(), scrollSuccess, legendarySpirit),
                            vegas == 0);
        }
    ...
    public static final boolean UseSkillBook(
        ...
        final Map<String, Integer> skilldata =
                MapleItemInformationProvider.getInstance().getSkillStats(toUse.getItemId());
        ...
        while (true) {
            ...
            if (CurrSkillData != null
                    && CurrSkillData.canBeLearnedBy(chr.getJob().getId())
                ...
                MapleInventoryManipulator.removeFromSlot(
                        c, GameConstants.getInventoryType(itemId), slot, (short) 1, false);
                break;
            }
        }
    ...
    public static final void removeItem(final MapleCharacter chr, final MapleMapItem mapitem, final MapleMapObject ob) {
        ...
        chr.getMap()
                .broadcastMessage(
                        MaplePacketCreator.removeItemFromMap(mapitem.getObjectId(), 2, chr.getId()),
                        mapitem.getPosition());
    ...
    public static final boolean getIncubatedItems(MapleClient c) {
        ...
        final int[] ids = {
            ...
            1012059,
            1012060,
            1012061, // pinocchio nose msea only.
            1332100,
            1382058,
            1402073,
            1432066,
        ...
        // out of 1000
        final int[] chances = {
            100, 100, 100, 100, 100, 1, 10, 10, 10, 10, 10, 10, 10, 5, 5, 5, 5, 5, 5, 5, 2, 10, 10,
            10, 10, 10, 10, 10, 10, 5, 5, 5, 5, 5, 5, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10,
            10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 5, 5, 10, 10, 10, 10, 10, 5, 5, 5,
            5, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 1, 2, 1, 2
        };
