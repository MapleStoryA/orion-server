...
public class PartyManager {
    ...
    public static void updateParty(int partyid, PartyHandlerUtils.PartyOperation operation, MaplePartyCharacter target) {
        MapleParty party = getParty(partyid);
        if (party == null) {
            return;
        }
        ...
        if (operation == PartyHandlerUtils.PartyOperation.LEAVE || operation == PartyHandlerUtils.PartyOperation.MOVE_MEMBER || operation == PartyHandlerUtils.PartyOperation.EXPEL) {
            ...
            if (chz > 0) {
                ...
                if (chr != null) {
                    ...
                    if (oldExped > 0 && operation != PartyHandlerUtils.PartyOperation.MOVE_MEMBER) {
                        // Broadcast to self
                        chr.getClient().getSession().write(MapleUserPackets.removeExpedition(operation == PartyHandlerUtils.PartyOperation.LEAVE ? 61 : 63));
                        // Broadcast to remaining member
                        expedPacket(oldExped, MapleUserPackets.expeditionNotice(operation == PartyHandlerUtils.PartyOperation.LEAVE ? 60 : 62, chr.getName()), new MaplePartyCharacter(chr));
                    }
                }
            }
        }
        ...
        for (MaplePartyCharacter partychar : party.getMembers()) {
            ...
            if (ch > 0) {
                MapleCharacter chr = WorldServer.getInstance().getChannel(ch).getPlayerStorage().getCharacterByName(partychar.getName());
                if (chr != null) {
                    if (operation == PartyHandlerUtils.PartyOperation.DISBAND || operation == PartyHandlerUtils.PartyOperation.DISBAND_IN_EXPEDITION) {
                        chr.setParty(null);
        ...
        if (oldExped > 0 && oldInd > -1 /*&& operation != PartyOperation.DISBAND && operation != PartyOperation.EXPEL && operation != PartyOperation.LEAVE*/) {
            expedPacket(oldExped, MapleUserPackets.expeditionUpdate(oldInd, party), (operation == PartyHandlerUtils.PartyOperation.LOG_ONOFF || operation == PartyHandlerUtils.PartyOperation.SILENT_UPDATE) ? target : null);
        }
    }
