...
function playerDisconnected(eim, player) {
	return 0;
	// return 0 - Deregister player normally Dispose instance if there are zero
	// player left
	// return x that is > 0 - Deregister player normally + Dispose instance if
	// there x player or below
	// return x that is < 0 - Deregister player normally + Dispose instance if
	// there x player or below, if it's leader = boot all
}

function monsterValue(eim, mobid) {
	// Invoked when a monster that's registered has been killed
	// return x amount for this player - "Saved Points"
	if (em.getProperty("balrogState").equals("1")
			&& eim.getMapInstance(0).getMonsterById(8830007) == null
			&& eim.getMapInstance(0).getMonsterById(8830008) == null
			&& eim.getMapInstance(0).getMonsterById(8830009) == null) {
...
function removePlayer(eim, player) {
	// Happens when the funtion
	// NPCConversationalManager.removePlayerFromInstance() is invoked
}
...
function checkHP(eim) {
	...
	if (hpDone > 120000) { // advance
		...
		eim
				.broadcastPlayerMsg(6,
						"Balrog was too strong and has overcome you.");
		end(eim);
	}
}